...
int x = 0;
    ...
    private void spBuildCFG(){
      ...
      {          
      dlmList.addElement(" ");
      dlmList.addElement("//NEWTWORK LAG FIXES");
      ...
      if(rdbScrollAtk.isSelected() && rdbStrafe.isSelected() && rdbScroll.isSelected() == false){
       dlmList.addElement(" ");
       dlmList.addElement("//Mutliple CFG's Below");
       dlmList.addElement("exec ScrollAtk.cfg");
       dlmList.addElement("exec Strafer.cfg");
       x = 1;
      }else if(rdbScrollAtk.isSelected() && rdbScroll.isSelected() && rdbStrafe.isSelected() == false){
       dlmList.addElement(" ");
       dlmList.addElement("//Mutliple CFG's Below");
       dlmList.addElement("exec ScrollAtk.cfg");
       dlmList.addElement("exec Legend.cfg");
       x = 2;
      }else if(rdbStrafe.isSelected() && rdbScroll.isSelected() && rdbScrollAtk.isSelected() == false){
       dlmList.addElement(" ");
       dlmList.addElement("//Mutliple CFG's Below");
       dlmList.addElement("exec Legend.cfg");
       dlmList.addElement("exec Strafer.cfg");
    ...
    {
    File Textfile = new File("/AppData/Roaming/autoexec.cfg");
       try
       {
    ...
    private void spCreateStrafer()
    {
    File Textfile = new File("Strafer.cfg");
       try
       {
           Textfile.createNewFile();           
       }
    ...
    private void spCreateLegend() {                                              
        File Textfile = new File("Legend.cfg");
        try{
           Textfile.createNewFile(); 
        }
        catch(Exception Ex) {
    ...
    private void spCreateFileScrollAtk() {                                              
        File Textfile = new File("ScrollAtk.cfg");
        try{
           Textfile.createNewFile(); 
        }
        catch(Exception Ex) {
    ...
    private void spCreateAuto(){
    {
     int intIndex = lstCFG.getModel().getSize();
        ...
        try
        {
            String strfilename="autoexec.cfg";
    ...
    private void spScrollAtk(){
    {
        ...
        try
        {
            String strfilename="ScrollAtk.cfg";
            FileWriter fw = new FileWriter(strfilename,true);
    ...
    private void spStrafer(){
    {
        ...
        try
        {
            String strfilename="Strafer.cfg";
            FileWriter fw = new FileWriter(strfilename,true);
            fw.write(arrContents[i]+ "\r\n");
            fw.close();
    ...
    private void initComponents() {
        ...
        lstCFG.setModel(new javax.swing.AbstractListModel<String>() {
            String[] strings = { "Please create file first, select options on the left which you would like to have in your .cfg", "There might be multiple that are created just cut from your desktop to the directory", " ", "If you are using Origin go to where you installed the game:", "~\\Origin\\Apex\\cfg", " ", "If you are using Steam got to where you intalled the game:", "~\\Steam\\steamapps\\common\\Apex Legends\\cfg", " " };
